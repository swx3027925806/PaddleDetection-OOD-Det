...
from .op_helper import jaccard_overlap, gaussian2D, gaussian_radius, draw_umich_gaussian, bbox_ioo, check_bound
...
__all__ = [
    ...
    'PadRGT', 'CopyPaste', 'BatchRandomResizeForSSOD'
]


@register_op
class CopyPaste(BaseOperator):
    """
    Copy-Paste

    Args:
        category_id (list): 选择增强种类的catID, 若为None表示全部增强
        ratio (float):  增强的概率
        obj_area_thresh (list): 面积阈值, 只增强此区间之内的标注框
        ioo_thresh (float): ioo阈值
        heavy_aug (bool): 是否启用加强copy-paste
        stride (bool): 滑动步长
    """
    ...
    def __init__(self,
        ...
        if category_id is None:
            # self.category_id = [i for i in range(2, 100)] # 这里只针对coco数据集
            self.category_id = [i for i in range(100)]
        else:
            self.category_id = category_id
        self.pro = pro
        self.obj_area_thresh = obj_area_thresh
        self.ioo_thresh = ioo_thresh
    ...
    def __call__(self, samples, context=None):
        ...
        wh = samples[0]['image'].shape[0]
        if self.stride is None:
            self.stride = wh // 10
        slide_cnt = wh // self.stride - 2
        cnt = slide_cnt * slide_cnt -1
        ...
        for i in lst:
            ...
            # 随机从不放回选取一张图作为src
            src_sample_index = random.choice(src_list)
            src_list.remove(src_sample_index)
            src_sample = samples[src_sample_index]
            # 获取src_bbox [x1, y1, x2, y2]
            src_bboxes = src_sample['gt_bbox'].copy()
            # 获取目标ID
            src_catId = src_sample['gt_class'].copy()
            # 获取src的图像信息
            src_image = src_sample['image'].copy()
            
            # src裁剪图像和标注信息
            copy_images_info = []
            for bbox, catID in zip(src_bboxes, src_catId):
                x1, y1, x2, y2 = map(int, bbox)
                sub_image = src_image[y1:y2, x1:x2, :]
                area_image = (x2 - x1) * (y2 - y1)
                  
                if self.heavy_aug:
                    # 过滤小目标检测框
                    if area_image < self.obj_area_thresh[0]:
                        continue
                    # 根据面积阈值缩小裁剪的图像, 也可以增加小目标的数量
                    while area_image > self.obj_area_thresh[1]:
                        sub_image = cv2.resize(sub_image, (0, 0), fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
                        h, w = sub_image.shape[:2]
                        area_image = w * h
                else:
                    # 过滤检测框
                    if area_image < self.obj_area_thresh[0] or area_image > self.obj_area_thresh[1]:
                        continue
                        
                copy_images_info.append([sub_image, catID])
            
            # 获得需要粘贴的检测框数目
            src_n = len(copy_images_info)
            for k in range(src_n):
                h, w = copy_images_info[k][0].shape[:2]
                slide_bbox = np.array([0., 0., w, h], dtype=np.float32)  # xywh

                j = -1
                while j < cnt:
                    j += 1
                    slide_bbox[:2] = self.stride * (j % slide_cnt + 1), self.stride * (j // slide_cnt + 1)
                    # 检测边框是否越界
                    if not check_bound(slide_bbox, wh, wh):
                        continue
                    temp_bbox = np.array([slide_bbox])
                    # 计算src_bbox与所有目的图像的bbox的交集占目的图像的bbox的比
                    ioo = bbox_ioo(bboxes.copy(), np.tile(temp_bbox, (bboxes.shape[0], 1)), wh, wh)
                    is_copy = np.where((ioo < self.ioo_thresh) | (ioo > 16.), True, False)
                    # 找到一个可以粘贴的位置
                    if np.all(is_copy):
                        # 更新参数
                        temp_bbox[:, 2:] += temp_bbox[:, :2]
                        bboxes = np.r_[bboxes, temp_bbox]
                        x1, y1, x2, y2 = map(int, temp_bbox[0])
                        samples[i]['image'][y1:y2, x1:x2, :] = copy_images_info[k][0]
                        gt_class = np.r_[gt_class, np.array([copy_images_info[k][1]])]
                        break
            # 更新参数
            samples[i]['gt_bbox'] = bboxes
            samples[i]['gt_class'] = gt_class
