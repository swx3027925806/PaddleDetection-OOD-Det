...
def bbox_ioo(box, src_box, w, h, mode='xywh'):
    """

    :param box: [[x, y, w, h], ...]
    :param src_box: [[x, y, w, h], ...]
    :param mode: bbox格式, xywh or x1y1x2y2
    :return: ioo
    """
    ...
    if mode == 'xywh':
        ...
        src_x1, src_x2 = src_box[:, 0], src_box[:, 0] + src_box[:, 2]
        src_y1, src_y2 = src_box[:, 1], src_box[:, 1] + src_box[:, 3]

    else:
        x1, x2 = box[:, 0], box[:, 2]
        y1, y2 = box[:, 1], box[:, 3]
        src_x1, src_x2 = src_box[:, 0], src_box[:, 2]
        src_y1, src_y2 = src_box[:, 1], src_box[:, 3]

    inner_x1 = np.maximum(x1, src_x1)
    inner_y1 = np.maximum(y1, src_y1)
    inner_x2 = np.minimum(x2, src_x2)
    inner_y2 = np.minimum(y2, src_y2)

    overlap = ((inner_x2 - inner_x1).clip(0)) * ((inner_y2 - inner_y1).clip(0))
    area = (x2 - x1) * (y2 - y1)
    area = area.clip(0)
    ioo = overlap / area
