...
class RandomFlip(BaseOperator):
    def __init__(self, prob=0.5, state=0):
        """
        Args:
            prob (float): the probability of flipping image
            state (int): 0: horizontal 1: vertical 2: vertical + horizontal
        """
    ...
    def apply_image(self, image, state):
        if state == 0:
            image = image[:, ::-1, :]
        elif state == 1:
            image = image[::-1, :, :]
    ...
    def apply_bbox(self, bbox, width, height, state):
        if state == 0:
            oldx1 = bbox[:, 0].copy()
            oldx2 = bbox[:, 2].copy()
            bbox[:, 0] = width - oldx2
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            state = random.randint(0, self.state+1)
            im = self.apply_image(im, state)
            if 'gt_bbox' in sample and len(sample['gt_bbox']) > 0:
                sample['gt_bbox'] = self.apply_bbox(sample['gt_bbox'], width, height, state)
...
@register_op
class BadWeather(BaseOperator):
    def __init__(self, weather=["fog", "rain", "cloud", "snow", "snowland"], n=(1, 3)):
        """
        Args:
            weather (list): the probability of flipping image
            state (int): 0: horizontal 1: vertical 2: vertical + horizontal
        """
        super(BadWeather, self).__init__()
        weather_dict = {
            "fog": iaa.weather.Fog(),
            "rain": iaa.weather.Rain(),
            "cloud": iaa.weather.Clouds(),
            "snow": iaa.weather.Snowflakes(),
            "snowland": iaa.weather.FastSnowyLandscape(),
        }
        self.weather = iaa.SomeOf(n, [weather_dict[i] for i in weather])
        if not (isinstance(weather, list)):
            raise TypeError("{}: input type is invalid.".format(self))

    def apply_image(self, image):
        return self.weather(images=image[None, :])[0]

    def apply(self, sample, context=None):
        sample['image'] = self.apply_image(sample['image'])
        return sample
